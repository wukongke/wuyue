  ...
  export default {
    ...
    created() {
      ...
      if (localBookReaderInfo && localBookReaderInfo[id]) {
        ...
        this.$store.dispatch('curChapterIndex', this.booksReadInfo[id].chapterIndex)
      } else {
        ...
        if (localBookReaderInfo) {
          ...
          this.$store.dispatch('curChapterIndex', 0)
        } else {  //第一次进入阅读
    ...
    methods: {
      ...
      getData(id, chapter) {
        this.loading = true
        axios.get(`${this.common.api}/reader?book=${id}&id=${chapter}`).then((res) => {
          this.loading = false  //获取完毕后隐藏动画
          this.title = res.data.data.title
          this.content = res.data.data.content.split('\n')
        })
      ...
      nextChapter() {
        this.$store.dispatch('nextChapter', 5000)
        this.saveBooksInfo()
        setTimeout(() => {
          document.body.scrollTop = 0
        }, 300)
      ...
      saveBooksInfo() {
        ...
        this.booksReadInfo[id] = {
          ...
          chapterIndex: this.curChapterIndex
        }
    ...
    computed: {
      ...mapState([
        'font_panel', 'bg_color', 'fz_size', 'bg_night', 'curChapter', 'curChapterIndex', 'windowHeight', 'list_panel'
      ])
    },
    watch: {
      //监听fz_size的值更改背景色
      ...
      // curChapterIndex(val, oldVal) {
      //   localEvent.StorageSetter('cur_chapter', this.curChapter)
      //   this.saveBooksInfo()
      //   this.getData(this.$route.params.id, this.curChapter)
      // }
    }
  }
</script>
