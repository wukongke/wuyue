  ...
  <div class="list-panel" @touchmove:prevent>
    <div class="list">
      ...
      <div class="list-content" @touchmove:prevent>
        <ul>
          <li v-for="(item,index) in chapterList" @click="jumpTo(item.id, index)">· {{index+1}}. {{item.title}}</li>
        </ul>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      jumpTo(id, index) {
        ...
        this.$store.dispatch('curChapterIndex', index)
      ...
      getList() {
        axios.get(`${this.common.api}/titles?id=${this.bookId}`).then(res => {
          // this.chapterList = res.data.data.titles.split('-')
          this.chapterList = res.data.data.titles
          this.$store.dispatch('curChapterList', this.chapterList)
        })
      }
    }
  }
